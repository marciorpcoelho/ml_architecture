...
def main():
    ...
    # seo_merge_function()
    seo_merge_function_2()
    # cost_calculation_function()


def cost_calculation_function():
    version = 'v10'
    emergency_cost = 5
...
def seo_merge_function():
    ...
    for week_day in days:
        for temperature_range in temps:
            df = pd.read_csv('output/seo_df_weekday_{}_temperaturerange_{}_{}.csv'.format(week_day, temperature_range, version), index_col=0)
            all_dfs = all_dfs.append(df)

    all_dfs = all_dfs[all_dfs['Stock Flag'] == 0]

    all_dfs.to_csv('output/seo_{}_top_5_part_refs_daily.csv'.format(version))
    ...
    all_dfs.to_csv('output/seo_{}_top5_part_refs.csv'.format(version))


def seo_merge_function_2():
    selected_parts = ['BM83.21.2.405.675', 'BM07.12.9.952.104', 'BM07.14.9.213.164', 'BM83.19.2.158.851', 'BM64.11.9.237.555']
    days = range(0, 5)
    ...
    for week_start_day in sel_week_days:
        ...
        for week_day in days:
            df = pd.read_csv('output/seo_df_week_start_{}_{}_{}_weekday_{}_temperaturerange_{}_{}.csv'.format(x.day, x.month, x.year, week_day, temperature_range[0], version), index_col=0)
            all_dfs = all_dfs.append(df)
        # print(all_dfs)
...
def results_merging_function_backup():
