    ...
    if '-' not in [sel_model] and '-' not in [sel_brand]:
        ...
        if status == 'optimal':
            ...
            if sel_configurations.shape[0]:
                ...
                st.write('Sugestão Encomenda (Score Inicial):', sel_configurations[['Sug.Encomenda'] + [proposals_col] + [stock_col] + [x for x in configuration_parameters if x not in 'PT_PDB_Model_Desc'] + ['Quantity_Sold'] + ['Average_Score_Euros']]
            ...
            if sel_configurations_v2.shape[0]:
                ...
                st.write('Sugestão Encomenda (Novo Score):', sel_configurations_v2[['Sug.Encomenda'] + [x for x in configuration_parameters] + temp_cols + ['Score']]
                         .rename(columns=options_file.column_translate_dict).reset_index(drop=True)
...
def co2_processing(df):
    ...
    df_filtered = df[df['Sales_Plan_Year'] == start_date_year]
    df_filtered.loc[:, 'Sales_Sum'] = df_filtered.loc[:, total_months_list[0:end_date_month_number - 1]].sum(axis=1)
    df_filtered.loc[:, 'Sales_Sum_Times_Co2_WLTP'] = df_filtered['Sales_Sum'] * df_filtered['WLTP_CO2']
    df_filtered.loc[:, 'Sales_Sum_Times_Co2_NEDC'] = df_filtered['Sales_Sum'] * df_filtered['NEDC_CO2']
    co2_wltp_sum = df_filtered['Sales_Sum_Times_Co2_WLTP'].sum(axis=0)
    co2_nedc_sum = df_filtered['Sales_Sum_Times_Co2_NEDC'].sum(axis=0)
    total_sales = df_filtered['Sales_Sum'].sum(axis=0)
...
def col_normalization(df, cols_to_normalize_in, cols_to_normalize_reverse_in):
    ...
    for col in cols_to_normalize_in:
        if col in cols_to_normalize_reverse_in:
            df[col + '_normalized'] = (df[col] - df[col].max()) / (df[col].min() - df[col].max())
        else:
